...
public class KolDondur : MonoBehaviour {
	...
	static double a3eski = fi - (a1eski + a2eski); //POTANSİYOMETREDEN ALINACAK
	...
	public double a1_istenilen;    // a2   açılarına göre 90 - 180  
	public double a2_istenilen;     //  a3   açılarına göre 90 - 270  // pratikte a3  120-240 gidiyor max robotun kolu!!!!!
	...
	void Update () {
		...
		{ 
           ...
           // if (a1yeni[c] >= 89.3) { Debug.LogError(" a1 89 oldu istenilen noktaya gidemez"); h = 0;  }
            ...
            // TAHANIN İSTEDİĞİ AÇI SANUÇLARI  // a1 0 - 90  // a4 0 - 180 

            a1_istenilen =a1yeni[c];    // a2   açılarına göre 90 - 180  
			a2_istenilen = 180 + a2yeni[c];     //  a3   açılarına göre 90 - 270  // pratikte a3  120-240 gidiyor max robotun kolu!!!!!
			a3_istenilen =  180+ a3yeni[c];
			a4_istenilen = a4yeni;



			// AÇI SONUÇLARI   // GARENİN BULDUGU AÇI SONUÇLARI 
			
            //a4yeni = a4yeni; değişmez zaten 
            /*
		...
		for (double a1 = 0; a1 <= 90; a1 = a1 + 0.05)
		{
			...
			if (Math.Round(Math.Abs(E3 - l2)) == Math.Round(0.0) || Math.Round(Math.Abs(E3 - l2)) <= Math.Round(2.0))
			{
				...
				if (Xcos > 0 && Ysin < 0)
				{
					...
					a4yeni = a4;
					if (a1yeni[j] >= 90 || a1yeni[j] <= 0 || a2yeni[j] >= 0 || a2yeni[j] <= -130 || a3yeni[j] <= -90 || a3yeni[j] >= 90)
					{
						//cout<<"Gidebilicegi acılar kısıtlandı ";
						a1yeni[j] = 1111;
						a2yeni[j] = 1111;
						a3yeni[j] = 1111;
					}
